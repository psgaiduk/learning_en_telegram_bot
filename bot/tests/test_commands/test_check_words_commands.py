...
class TestCheckWordsCommand:
    ...
    @mark.asyncio
    async def test_handle_check_words_after_read(
        self, mock_check_word_service, mock_send_delete_message, mock_delete_message
    ):
        chat_id = 1
        user = User(id=chat_id, is_bot=False, first_name="Test User")
    ...
    async def test_handle_check_word_click_known(
        ...
        if is_update_history is True:
            ...
            mock_check_word_service.return_value.do.assert_awaited_once_with()
            mock_delete_message.assert_called_once_with(message=mock_callback, state=state)
        else:
