...
class TestCreateHistoryWordAPI:
    ...
    def test_create_word_history(self):
        ...
        with db_session() as db:
            ...
            assert words_history.created_at == datetime.strptime(
                response["detail"]["created_at"], "%Y-%m-%dT%H:%M:%S.%f"
            )
            assert words_history.updated_at == datetime.strptime(
                response["detail"]["updated_at"], "%Y-%m-%dT%H:%M:%S.%f"
            )
