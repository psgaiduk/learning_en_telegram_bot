...
class TestGetHistoryWordAPI:
    ...
    def test_get_word_history_filter_is_known(self, history_all_words_mock):
        ...
        with db_session() as db:
            ...
            all_history_user_words_query = (
                db.query(UsersWordsHistory).filter(UsersWordsHistory.telegram_user_id == telegram_user_id).all()
            )
